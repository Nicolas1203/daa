...
class StyleAugment(nn.Module):
    ...
    """
    def __init__(self, input_size=32, transfer_size=128, **kwargs):
        super().__init__()
    ...
    def forward(self, content_im, style_im, **kwargs):
        with torch.no_grad():
            ...
            output = Resize((self.input_size, self.input_size))(output)
            return output
    ...
    def adaptive_instance_normalization(self, content_feat, style_feat, *kwargs):
    ...
    def style_transfer(self, vgg, decoder, content, style, alpha=1.0,
                    interpolation_weights=None, *kwargs):
        with torch.no_grad():
