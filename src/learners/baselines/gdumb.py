...
class GDUMBLearner(CELearner):
    ...
    def init_tag(self):
        ...
        """
        self.params.tag += f"_m{self.params.mem_size}epoch{self.mem_epochs}"
    ...
    def train_on_memory(self, **kwargs):
        ...
        for e in range(self.mem_epochs):
            ...
            for j in range(len(mem_y) // batch_size):
                _, logits = self.model(self.transform_train(mem_x[batch_size * j:batch_size * (j + 1)]))
    ...
    def evaluate(self, dataloaders, task_id, **kwargs):
        ...
        acc, fgt = super().evaluate(dataloaders, task_id)
        return acc, fgt
